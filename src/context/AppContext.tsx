...
export const AppProvider: React.FC<{
  ...
  const [isSupabaseConnected, setIsSupabaseConnected] =
    useState<boolean>(isSupabaseConfigured());

  const [curatedCollections] = useState<Collection[]>(
    CURATED_COLLECTIONS
  );
  ...
  const [favoriteIds, setFavoriteIds] = useState<string[]>(
    () => {
      const saved =
        localStorage.getItem('ws_favorite_ids');
  ...
  const [downloadHistoryIds, setDownloadHistoryIds] =
    useState<string[]>(() => {
      const saved =
        localStorage.getItem('ws_download_history');
  ...
  const [userCollections, setUserCollections] =
    useState<Collection[]>(() => {
      const saved =
        localStorage.getItem('ws_user_collections');
  ...
  const isAdmin =
    authUser?.id === ADMIN_SUPABASE_UID;
  ...
  const uploadWallpaperWithFile =
    async (
      ...
      addToast(
        'Uploading wallpaper image to Supabase Storage...',
        'info'
      );
  ...
  const seedSupabaseDatabase =
    async () => {
      if (
        ...
        addToast(
          'Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your environment first',
          'error'
        );
